...
function TurtleRP.tooltip_events()
    ...
    for i = 1, 11 do
        local lfontName, lfontHeight, lflag = getglobal(
                                                  "GameTooltipTextLeft" .. i):GetFont()
        ...
        local rfontName, rfontHeight, rflag = getglobal(
                                                  "GameTooltipTextRight" .. i):GetFont()
    ...
    -- Custom scripts
    local defaultTooltipClearedScript = TurtleRP.gameTooltip:GetScript(
                                            "OnTooltipCleared")
    TurtleRP.gameTooltip:SetScript("OnTooltipCleared", function()
        ...
        if defaultTooltipClearedScript then defaultTooltipClearedScript() end
    end)
    ...
    TurtleRP.gameTooltip:SetScript("OnShow", function()
        ...
        if defaultTooltipShowScript then defaultTooltipShowScript() end
    end)
...
function TurtleRP.buildTooltip(playerName, targetType)
    ...
    if characterInfo and characterInfo["keyM"] then locallyRetrievable = true end

    -- Getting details for character
    local nsfw = locallyRetrievable and characterInfo["nsfw"] or nil
    local fullName = locallyRetrievable and characterInfo["full_name"] or
                         UnitName(targetType)
    local race = locallyRetrievable and characterInfo["race"] or
                     UnitRace(targetType)
    local raceKey = locallyRetrievable and characterInfo["raceKey"] or
                        localize(UnitRace(targetType))
    local class = locallyRetrievable and characterInfo["class"] or
                      UnitClass(targetType)
    local classKey = locallyRetrievable and characterInfo["classKey"] or
                         localize(UnitClass(targetType))
    local class_color = locallyRetrievable and characterInfo['class_color'] or
                            TurtleRPClassData[classKey][4]
    local guildName = nil
    local guildRank = nil
    local queriedLevel = ""
    local icon = locallyRetrievable and characterInfo['icon'] or nil
    local currently_ic = locallyRetrievable and characterInfo['currently_ic'] or
                             nil
    local ic_info = locallyRetrievable and characterInfo['ic_info'] or nil
    local ic_pronouns = locallyRetrievable and characterInfo['ic_pronouns'] or
                            nil
    local ooc_info = locallyRetrievable and characterInfo['ooc_info'] or nil
    local ooc_pronouns = locallyRetrievable and characterInfo['ooc_pronouns'] or
                             nil
    ...
    local blankLine = " "
    local statusText = TurtleRP.getStatusText(currently_ic, ICOn, ICOff,
                                              whiteColor)
    local level = queriedLevel == -1 and "??" or queriedLevel
    local levelAndStatusText = localize("tooltip.level") .. " " .. level ..
                                   statusText
    local raceAndClassText = whiteColor .. race .. " " .. thisClassColor ..
                                 class
    local ICandPronounsText = localize("profile.basic.icInfo") ..
                                  TurtleRP.getPronounsText(ic_pronouns,
                                                           pronounColor)
    local OOCandPronounsText = localize("profile.basic.oocInfo") ..
                                   TurtleRP.getPronounsText(ooc_pronouns,
                                                            pronounColor)

    -- Modify tooltip
    if nsfw == "1" and TurtleRPSettings["show_nsfw"] == "0" then
        TurtleRP.gameTooltip:AddLine(blankLine)
        TurtleRP.gameTooltip:AddLine(localize("tooltip.nsfwProfile"))
        TurtleRP.gameTooltip:AddLine(localize("tooltip.nsfwProfileInfo1"), 0.7,
                                     0.7, 0.7)
        TurtleRP.gameTooltip:AddLine(localize("tooltip.nsfwProfileInfo2"), 0.7,
                                     0.7, 0.7)
    ...
    if getglobal("GameTooltipTextLeft1"):GetText() == nil then
        TurtleRP.gameTooltip:AddLine(thisClassColor .. titleExtraSpaces ..
                                         fullName)
    end
    ...
    getglobal("GameTooltipTextLeft" .. l):SetText(thisClassColor ..
                                                      titleExtraSpaces ..
                                                      fullName)

    l = l + 1
    if guildName then
        getglobal("GameTooltipTextLeft" .. l):SetText(guildColor ..
                                                          guildExtraSpaces ..
                                                          "<" .. guildRank ..
                                                          " of " .. guildName ..
                                                          ">")
    ...
    if getglobal("GameTooltipTextLeft3"):GetText() == nil then
        ...
        if guildName ~= nil then
            if string.find(getglobal("GameTooltipTextLeft3"):GetText(),
                           guildName) then
    ...
    if locallyRetrievable then
        ...
        if icon ~= nil and icon ~= "" then
            TurtleRP_Tooltip_Icon:SetPoint("TOPLEFT", GameTooltipTextLeft1,
                                           "TOPLEFT")
            TurtleRP_Tooltip_Icon:SetFrameStrata("tooltip")
            TurtleRP_Tooltip_Icon:SetFrameLevel(99)
            TurtleRP_Tooltip_Icon_Icon:SetTexture(
                "Interface\\Icons\\" .. TurtleRPIcons[tonumber(icon)])
